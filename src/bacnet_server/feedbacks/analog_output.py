from bacpypes.local.object import AnalogOutputCmdObject, AnalogValueCmdObject
    ...
    def check_feedback(self, old_value, new_value):
        ...
        with self.__app_context():
            ...
            if setting.mqtt.publish_value:
                publish_value_to_mqtt(self, priority_array_updated)
...
def publish_value_to_mqtt(point, priority_array):
    object_identifier = create_object_identifier(point.objectIdentifier[0], point.objectIdentifier[1])
    ...
    object_type: str = point.objectIdentifier[0]
    topic_suffix: tuple = (object_type, object_identifier, point.profileName, point.objectName)
